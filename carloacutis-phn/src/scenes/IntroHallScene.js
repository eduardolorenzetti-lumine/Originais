...
import { clearScoreboard } from "../scoreboard.js?v=carlo-infinite-play-1";
...
export class IntroHallScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.pressStart = this.add.image(480, 360, "title-press-start")
      .setDisplaySize(980, 720)
      .setAlpha(0);
    this.resetNotice = this.add.text(480, 632, "SCOREBOARD RESET", {
    ...
    this.input.keyboard.on("keydown", this.handleResetShortcut, this);
    this.events.once("shutdown", () => {
      this.input.keyboard.off("keydown", this.handleResetShortcut, this);
    });
  ...
  handleResetShortcut(event) {
    ...
    clearScoreboard();
    this.tweens.killTweensOf(this.resetNotice);
    this.resetNotice.setAlpha(1);
    this.tweens.add({
      targets: this.resetNotice,
      alpha: 0,
      duration: 900,
      delay: 900,
      ease: "Linear",
