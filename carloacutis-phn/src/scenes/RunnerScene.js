import { GAME_WIDTH, LANES, MAPS } from "../config.js?v=carlo-infinite-play-1";
import {
...
import { getRecordThresholds } from "../scoreboard.js?v=carlo-infinite-play-1";
...
export class RunnerScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.distanceMeterScale = 132;
    this.recordThresholds = getRecordThresholds();
    this.crossedRecordThresholds = new Set();
    ...
    this.distanceMeterScale = this.carlo.displayWidth;
    ...
    this.distanceText = this.add.text(912, 42, "0 M", {
      fontFamily: '"Press Start 2P", monospace',
      fontSize: "18px",
      color: "#fff6dd",
      stroke: "#1b2130",
      ...
      .setOrigin(1, 0)
      .setDepth(950);
    this.recordNotice = this.add.text(480, 132, "", {
      fontFamily: '"Press Start 2P", monospace',
      fontSize: "20px",
      color: "#fff6dd",
      stroke: "#1b2130",
  ...
  showRecordNotice(message) {
    this.tweens.killTweensOf(this.recordNotice);
    this.recordNotice
      .setText(message)
      .setAlpha(0)
      .setY(132);
    ...
    this.tweens.add({
      ...
      yoyo: true,
      hold: 1100,
      onComplete: () => this.recordNotice.setAlpha(0),
    });
  }

  updateDistanceReadout() {
    const nextMeters = Math.floor(this.distance / this.distanceMeterScale);
    if (nextMeters === this.distanceMeters) {
      return;
    }

    this.distanceMeters = nextMeters;
    this.distanceText.setText(`${this.distanceMeters} M`);

    this.recordThresholds.forEach((entry, index) => {
      if (
        this.distanceMeters > entry.distance &&
        !this.crossedRecordThresholds.has(index)
      ) {
        this.crossedRecordThresholds.add(index);
        this.showRecordNotice(entry.isTopRecord ? "NEW TOP RECORD" : "NEW RECORD");
      }
    });
  }
